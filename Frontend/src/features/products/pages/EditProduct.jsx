...
const EditProduct = () => {
            ...
            .then(data => {
                if (data) {
                    ...
                    if (data.images && data.images.length > 0) {
                        setImages(data.images.map(img => ({ file: null, preview: img.url, isExisting: true })));
                    }
                }
            })
    ...
    const addFiles = (files) => {
        ...
        setImages(prev => {
            ...
            const newImages = toAdd.map(file => ({ file, preview: URL.createObjectURL(file), isExisting: false }));
    ...
    const inputClass = "w-full bg-transparent outline-none py-4 text-sm transition-colors duration-300 placeholder:text-[#525252]";
    const inputStyle = { color: '#ffffff', borderBottom: '1px solid #2d2d2d', fontFamily: "'Inter', sans-serif" };
    const handleFocus = (e) => { e.target.style.borderBottomColor = '#10b981'; };
    const handleBlur = (e) => { e.target.style.borderBottomColor = '#2d2d2d'; };
            ...
            >
                <div className="max-w-6xl mx-auto px-8 lg:px-16 xl:px-24">
                    ...
                    <div className="pt-10 pb-0 flex justify-between items-center">
                        ...
                        <button onClick={() => navigate(-1)} className="px-4 py-2 bg-white/10 rounded-lg hover:bg-white/20 transition text-sm">
                            Cancel
                        </button>
                    </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 lg:items-start">
                            ...
                            <div className="flex flex-col gap-12">
                                ...
                                <div className="flex flex-col gap-3">
                                    <label className="text-[10px] uppercase tracking-[0.2em] font-medium" style={{ color: '#d3c5ac' }}>
                                        Price
                                    </label>
                                    <div className="flex gap-5 items-end">
                                        <div className="flex flex-col gap-1 flex-[3]">
                                            <span className="text-[9px] uppercase tracking-[0.18em]" style={{ color: '#a3a3a3' }}>Amount</span>
                                            <input
                                                id="cp-priceAmount"
                                                type="number"
                                                name="priceAmount"
                                        ...
                                        <div className="flex flex-col gap-1 flex-[1]">
                                            <span className="text-[9px] uppercase tracking-[0.18em]" style={{ color: '#a3a3a3' }}>Currency</span>
                                            <select
                                                id="cp-priceCurrency"
                                                name="priceCurrency"
                                            ...
                                            >
                                                {CURRENCIES.map(c => (
                                                    <option key={c} value={c} style={{ backgroundColor: '#1e1e1e', color: '#ffffff' }}>{c}</option>
                                                ))}
                                            </select>
                                        </div>
                                    </div>
                                ...
                                {images.length < MAX_IMAGES && (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            backgroundColor: isDragging ? 'rgba(16,185,129,0.04)' : 'transparent'
                                        }}
                                    >
                                        <div
                                            className="w-10 h-10 flex items-center justify-center border transition-colors duration-300"
                                            style={{ borderColor: isDragging ? '#10b981' : '#2d2d2d', color: isDragging ? '#10b981' : '#a3a3a3' }}
                                        >
                                        ...
                                        <div className="text-center">
                                            <p className="text-sm leading-relaxed" style={{ color: '#d3c5ac' }}>
                                                Drop images here or{' '}
                                                <span style={{ color: '#10b981', textDecoration: 'underline', textUnderlineOffset: '2px' }}>
                                                    tap to upload
                                                </span>
                                            </p>
                                            <p className="text-[10px] uppercase tracking-[0.15em] mt-2" style={{ color: '#a3a3a3' }}>
                                        ...
                                        {images.map((img, index) => (
                                            ...
                                            >
                                                ...
                                                <button
                                                    ...
                                                    style={{ backgroundColor: 'rgba(19,19,19,0.7)', color: '#ffffff' }}
                                                >
                                                    Remove
                                                </button>
                                            </div>
